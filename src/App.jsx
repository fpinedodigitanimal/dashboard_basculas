...
function App() {
  ...
  const filterAlertsBySelection = (allAlerts, enabledScales) => {
    ...
    return allAlerts.filter(alert => {
      ...
      const alertScaleId = alert.scale_id || 
  ...
  return (
    <ProtectedRoute>
      <div className="h-screen flex flex-col bg-[#f8faf5] overflow-hidden">
        <Header onSettingsClick={() => setShowSettings(!showSettings)} />
      
      {/* Botones de filtro */}
      <div className="bg-white border-b border-gray-200 shadow-sm">
        <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-8 py-1.5">
          <div className="flex items-center gap-3 flex-wrap text-xs">
            ...
            <div className="flex items-center gap-1.5 border-r border-gray-300 pr-3">
              ...
              {!isToday && (
                <button
                  onClick={goToToday}
                  className="ml-0.5 px-1.5 py-0.5 text-[10px] font-medium text-primary-600 hover:bg-primary-50 rounded transition-colors"
                >
                  Hoy
                </button>
              )}
            ...
            <div className="flex items-center gap-1.5 flex-wrap">
              <span className="text-[10px] text-gray-600 font-medium mr-0.5">Filtro:</span>
              <button
      ...
      <main className="flex-1 overflow-y-auto max-w-full mx-auto px-4 sm:px-6 lg:px-8 py-1">
        {/* KPI Cards */}
        <KPICards />
        ...
        {loading && !data ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-1.5 animate-fade-in">
            {/* Columna izquierda: Grid de gráficos - 10 columnas */}
            <div className="lg:col-span-10 flex flex-col gap-1.5">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-9 gap-1.5">
                {/* Heatmap - 7/9 */}
                <div className="lg:col-span-7">
                  <HeatmapChart data={data?.heatmapData} filter={filter} selectedDate={selectedDate} />
                </div>
                
                {/* Distribución de Pesos - 2/9 */}
            ...
            {/* Columna derecha: Tablas apiladas - 2 columnas */}
